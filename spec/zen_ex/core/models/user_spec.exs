defmodule ZenEx.Model.UserSpec do
  ...
  describe "list" do
    before do: allow HTTPClient |> to(accept :get, fn(_) -> response_users() end)
    it do: expect Model.User.list |> to(eq users())
  end

  describe "show" do
    before do: allow HTTPClient |> to(accept :get, fn(_) -> response_user() end)
    it do: expect Model.User.show(user().id) |> to(eq user())
  end

  describe "create" do
    before do: allow HTTPClient |> to(accept :post, fn(_, _) -> response_user() end)
    it do: expect Model.User.create(user()) |> to(be_struct User)
  end

  describe "update" do
    before do: allow HTTPClient |> to(accept :put, fn(_, _) -> response_user() end)
  ...
  describe "destroy" do
    before do: allow HTTPClient |> to(accept :delete, fn(_) -> response_user() end)
