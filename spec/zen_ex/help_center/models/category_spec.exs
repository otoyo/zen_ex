defmodule ZenEx.HelpCenter.Model.CategorySpec do
  ...
  describe "list" do
    before do: mock(fn %{method: :get, url: _} -> %Tesla.Env{status: 200, body: response_categories()} end)
  ...
  describe "show" do
    before do: mock(fn %{method: :get, url: _} -> %Tesla.Env{status: 200, body: response_category()} end)
    it do: expect Model.Category.show("en-us", category().id) |> to(eq category())
  end

  describe "create" do
    before do: mock(fn %{method: :post, url: _} -> %Tesla.Env{status: 200, body: response_category()} end)
    it do: expect Model.Category.create(category()) |> to(be_struct Category)
  end

  describe "update" do
    before do: mock(fn %{method: :put, url: _} -> %Tesla.Env{status: 200, body: response_category()} end)
