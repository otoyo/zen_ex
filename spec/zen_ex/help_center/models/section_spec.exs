defmodule ZenEx.HelpCenter.Model.SectionSpec do
  ...
  describe "list" do
    before do: mock(fn %{method: :get, url: _} -> %Tesla.Env{status: 200, body: response_sections()} end)
  ...
  describe "show" do
    before do: mock(fn %{method: :get, url: _} -> %Tesla.Env{status: 200, body: response_section()} end)
    it do: expect Model.Section.show("en-us", section().id) |> to(eq section())
  end

  describe "create" do
    before do: mock(fn %{method: :post, url: _} -> %Tesla.Env{status: 200, body: response_section()} end)
    it do: expect Model.Section.create(section()) |> to(be_struct Section)
  end

  describe "update" do
    before do: mock(fn %{method: :put, url: _} -> %Tesla.Env{status: 200, body: response_section()} end)
